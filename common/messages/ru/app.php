<?php
return [
    'Jobboard' => "JOBBOARD",
    'Home' => 'Главная страница',
    'About' => 'о Cайте',
    'Blog' => 'Блог',
    'Contact' => 'Контакт',
    'Login' => 'Авторизоваться',
    'Signup' => "Зарегистрироваться",
    'LoginTitle' => "Войти в JobBoard",
    'SignupTitle' => "Зарегистрироваться в JobBoard",
    'Logout (' => 'Выйти (',
    'ID' => 'Id',
    'User Id' => 'ID пользователя',
    'Name' => 'Имя',
    'Director Name' => 'Имя директора',
    'Region ID' => 'ID региона',
    'City ID' => 'ID города',
    'image' => 'Изображение',
    'Status' => 'Статус',
    'Website Template by Colorlib' => 'Шаблон сайта от Colorlib',
    'Edit' => 'Редактировать',
    'Save' => 'Сохранить',
    'Username' => 'Усернаме',
    'Password' => 'Пассwорд',
    'email' => 'Эмаил',
    'Firstname' => 'Имя',
    'Lastname' => 'Фамилия',
    'Patronymic' => 'Отчество',
    'Birthdate' => 'День рождения',
    'Gender' => 'Пол',
    'Nationality ID' => 'Идентификатор гражданства',
    'Address' => 'Адрес',
    'Phone' => 'Телефон',
    'Photo' => 'Фото',
    'Edit company information' => 'Редактировать информацию о компании',
    'Edit worker information' => 'Редактировать информацию о работнике',
    'Employer' => 'Работодатель',
    'Job seeker' => 'Соискатель',
    'Are you an employer or a job seeker?' => 'Вы работодатель или ищущий работу?',
    'Region' => 'Область',
    'City / District' => 'Город / Район',
    'Your data has been successfully modified' => 'Ваши данные были успешно изменены',
    'An error occurred while modifying your data' => 'Произошла ошибка при изменении ваших данных',
    ' personal information' => 'Персональные данные',
    'Cabinet' => 'Кабинет',
    'About Us' => 'О нас',
    'Post a Job' => 'Опубликовать вакансию',
    'Vacancy views' => 'Просмотры вакансий',
    'Job Listed' => 'Вакансии указаны',
    'Company name' => 'Название компании',
    'User name' => 'Имя пользователя',
    'Profession name' => 'Название профессии',
    'View Vacancies' => 'Посмотреть вакансии',
    'Add Vacancies' => 'Добавить вакансии',
    'Create Worker' => 'Я сделаю работа',
    "Company informations" => 'Информация о компании',
    'Description Uz' => 'Описание Уз',
    'Description En' => 'Описание En',
    'Description Ru' => 'Описание RU',
    'Description Cyrl' => 'Описание Cyrl',
    'Job Type' => 'Тип задания',
    'Number of jobs' => 'Количество рабочих мест',
    'Salary' => 'Зарплата',
    'Experience' => 'Опыт',
    'Telegram' => 'Телеграм',
    'Views' => 'Просмотры',
    'Deadline' => 'Крайний срок',
    'Create Vacancy' => 'Создать вакансию',
    'Job Details' => 'Детали работы',
    'Select the job type' => 'Выберите тип работы',
    'Choose a career type' => 'Выберите тип карьеры',
    'Select a region' => 'Выберите регион',
    'Select a city' => 'Выберите город',
    'Update Company' => 'Обновить компанию',
    'Save Job' => 'Сохранить работу',
    'Apply now' => 'Применить сейчас',
    'Here is the picture' => 'Вот картинка',
    'Job Description' => 'Описание работы',
    'Job Summary' => 'Резюме вакансии',
    'Published on' => 'Опубликован в',
    'Vacancy:' => 'Вакансия:',
    'Vacancies' => 'Вакансия',
    'Employment / Status:' => 'Работа / Статус:',
    'Job Location:' => 'место работы:',
    'Application Deadline:' => 'Крайний срок подачи заявок:',


    //    site footer start
    'Search Trending' => 'Поиск в тренде',
    'Web Design' => 'Веб-дизайн',
    'Graphic Design' => 'Графический дизайн',
    'Web Developers' => 'Веб-разработчики',
    'Python' => 'Python',
    'HTML5' => 'HTML5',
    'CSS3' => 'CSS3',
    'Career' => 'Карьера',
    'Resources' => 'Ресурсы',
    'Support' => 'Служба поддержки',
    'Privacy' => 'Конфиденциальность',
    'Terms of Service' => 'условия обслуживания',
    //    Pagination
    'Prev' => '',
    'Next' => '',

    // worker-form translate
    'Select a language' => 'Выберите язык',

    //vacancy orderCv list
    'Cv list' => 'Список резюме',
    'Vacancy name' => 'Название вакансии',
    'Company View' => 'Взгляд компании',
    'Worker View' => 'Просмотр рабочих',
    'Created_at' => 'Дата создания',
    'Date approval' => 'Дата утверждения',
    'Cv' => 'Резюме',
    'Show' => 'Показать',
    'You have received messages' => 'Вы получили сообщения',
    'The Easiest Way To Get Your Dream Job' => 'Самый простой способ получить работу своей мечты',
    'Trending Keywords:' => 'Популярные ключевые слова:',
    'Select a profession' => 'Выберите профессию',
    'Search Job' => 'Поиск работы',
    'Select a gender' =>'Выберите пол',
    'JobBoard Site Stats' => 'Статистика сайта JobBoard',
    'Not included' => 'Не включено',
    'Trending vacancy' => 'Актуальная вакансия',
    'Partners' => 'Партнеры',
    'Show more' => 'Показать больше',
    'Salary begin' => 'Начало зарплаты',
    'Salary end' => 'Конец зарплаты',
    'Search' => 'Поиск',
    'Reset'=>'Вернуться к',

];
?>